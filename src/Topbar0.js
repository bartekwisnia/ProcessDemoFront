    ...
    <div className="level">
      ...
      {sim_view ?
        ...
        {!sim_view ?
          ...
          <div className="level-item">
            <div className="field is-grouped is-grouped-multiline">
              <div className={demo_step === 9 ? "control mb-0 p-1 has-background-warning" : "control mb-0 p-1"}>
              ...
              <div className="control mb-0 p-1">
                ...
                <div className="buttons level-item are-small">
                  <button className="button" onClick={props.saveChanges}>{sim_view ? "SAVE_STATE" : "SAVE"}</button>
                    <button className="button" onClick={() => getData('plantdata/'+plant.id+'/', token).then(data => props.readPlantAutomation(data))}>
                {sim_view ? ((save_date_time) ? "RESTORE STATE FROM " + save_date_time : "RESTART") : "DISCARD"}
              </button>
            </div>
                        :
                        <div className="level-item mb-0 p-1">
                          <p>synchronized</p>
                        </div>}
            </div>
            </div>
